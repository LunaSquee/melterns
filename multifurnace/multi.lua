...
function multifurnace.api.continuous_floor(min, dimensions)
...
function multifurnace.api.continuous_sides(min, dimensions, max_height)
	local ports = {}
	local height = 0
	...
	for y = 0, max_height do
		...
		for x = 0, dimensions.x do
			...
			local far_wall = vector.add(min, {x = x, y = y, z = dimensions.z + 1})
		...
		for z = 0, dimensions.z do
			...
			local far_wall = vector.add(min, {x = dimensions.x + 1, y = y, z = z})
...
function multifurnace.api.structure_detect(node, pos)
	...
	local center, min, max = multifurnace.api.detect_center(back, 16)
	...
	local floor = multifurnace.api.continuous_floor(min, dimensions)
	if not floor then
		-- core.debug("Floor is not continuous")
		return nil, {}
	end

	local max_height, ports = multifurnace.api.continuous_sides(min, dimensions, 16)
...
function multifurnace.api.remove_port(pos)
	...
	for i,k in ipairs(ctrl_info.ports) do
		if vector.equals(k, pos) then
			indexof = i
		end
	end

	if indexof >= 0 then
		table.remove(ctrl_info.ports, indexof)
	end

	multifurnace.api.check_controller(ctrl_pos)
end
